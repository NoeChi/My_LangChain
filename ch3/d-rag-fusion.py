...
# RRF 演算法：https://blog.csdn.net/yanqianglifei/article/details/153690220
# k 是一個調整參數，通常設置為60
def reciprocal_rank_fusion(results: list[list], k=60):
    ...
    # print('RRF input shape:',len(results), len(results[0]))
    fused_scores = {}
    documents = {}
    for docs in results:
        ...
        for rank, doc in enumerate(docs):
            ...
            fused_scores[doc_str] += 1 / (rank + k) # rank越小，分數越高 ; 跨不同的 query，分數也會越高
...
# 1個query進去，變成4個query出來
# 每一個query搜尋5個文件
# 最後用reciprocal rank fusion把4*5個文件重新排序
